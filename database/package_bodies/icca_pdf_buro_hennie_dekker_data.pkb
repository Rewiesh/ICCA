create or replace package body icca_pdf_buro_hennie_dekker_data as
   ...
   -- c_base_url constant varchar2(100) := 'https://icca-dashboard.maxapex.net'; -- Aanpassen naar echte URL indien nodig
  ...
  -- LEFT JOIN om ALLE types te tonen (ook met 0%)
      ...
      for r in (
         ...
         on doc.id = err.log_book_image_id
          where fom.adt_id = p_adt_id
            ...
            and doc.file_url is not null  -- Extra check voor zekerheid
         ...
         -- Voeg page break toe elke 6 foto's (2 rijen van 3 kolommen)
         -- BELANGRIJK: Dit moet VOOR de foto komen waar de nieuwe pagina begint
         l_obj.put(
            'page_break',
            case
               when mod(
   ...
   begin
      for r in (
        ...
        -- Haal file_url op uit icca_documents
         begin
            select doc.file_url
              into l_file_url
              from icca_documents doc
        ...
        -- Alleen toevoegen als er een geldige URL is
         if l_file_url is not null then
            l_count := l_count + 1;
            l_obj := json_object_t();
            l_obj.put(
               'url',
               c_base_url || l_file_url
            );
            l_obj.put(
               'beschrijving',
               nvl(
